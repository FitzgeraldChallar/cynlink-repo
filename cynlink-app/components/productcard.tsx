    ...
    <div className="text-sm border border-dark_blue/20 rounded-md bg-white group">
      ...
      <div className="p-3 flex flex-col">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items- gap-0.5">
            {[...Array(5)].map((_,index)=>(
              <StarIcon 
                key={index}
                size={8} 
        ...
        <div className="flex items-center gap-2.5 ">
          ...
          <p className={` ${product?.stock === 0 ? "text-red-600" : "text-shop_light_gold/80 font-semibold"}`}>
        ...
        <PriceView
          ...
          className="text-sm"
        />
        <AddToCartButton product={product} className="w-full sm:w-36 rounded-full" />
      </div>
    </div>
  );
};
